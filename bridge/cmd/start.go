...
func Start() {
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{Destination: &conf.Host, Name: "host", Value: "", Usage: "The hostname of the Omada Controller, including protocol.", Sources: cli.EnvVars("OMADA_HOST")},
		...
		&cli.StringFlag{Destination: &conf.SystemType, Name: "system-type", Value: config.SystemTypeAuto, Usage: "Omada system type: auto, standard, or fusion.", Sources: cli.EnvVars("OMADA_SYSTEM_TYPE")},
		&cli.StringFlag{Destination: &conf.OpenAPIAuth, Name: "openapi-auth", Value: config.OpenAPIAuthAuto, Usage: "OpenAPI authentication mode: auto, client_credentials, web_session, or disabled.", Sources: cli.EnvVars("OMADA_OPENAPI_AUTH")},
		...
		&cli.IntFlag{Destination: &conf.CacheTTL, Name: "cache-ttl", Value: 5, Usage: "Cache Omada API fetch results for this many seconds. Set 0 to disable.", Sources: cli.EnvVars("OMADA_CACHE_TTL")},
		...
		&cli.IntFlag{Destination: &conf.InsightWindowSeconds, Name: "insight-window-seconds", Value: 86400, Usage: "DPI insight query window in seconds.", Sources: cli.EnvVars("OMADA_INSIGHT_WINDOW_SECONDS")},
		&cli.IntFlag{Destination: &conf.InsightApplicationLimit, Name: "insight-application-limit", Value: 50, Usage: "Maximum DPI application metric series to export. Set 0 to disable application metrics.", Sources: cli.EnvVars("OMADA_INSIGHT_APPLICATION_LIMIT")},
		...
		&cli.IntFlag{Destination: &conf.MQTTInterval, Name: "mqtt-interval", Value: 60, Usage: "MQTT publish interval in seconds.", Sources: cli.EnvVars("OMADA_MQTT_INTERVAL")},
		...
		&cli.IntFlag{Destination: &conf.MQTTExpireAfter, Name: "mqtt-expire-after", Value: 180, Usage: "Home Assistant sensor expire_after value in seconds. Set 0 to disable.", Sources: cli.EnvVars("OMADA_MQTT_EXPIRE_AFTER")},
