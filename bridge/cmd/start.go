...
func Start() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.BoolFlag{Destination: &conf.TrackInsightMetrics, Name: "track-insight-metrics", Value: false, Usage: "Export optional DPI insight metrics from Omada Web API.", EnvVars: []string{"OMADA_TRACK_INSIGHT_METRICS"}},
		&cli.IntFlag{Destination: &conf.InsightWindowSeconds, Name: "insight-window-seconds", Value: 86400, Usage: "DPI insight query window in seconds.", EnvVars: []string{"OMADA_INSIGHT_WINDOW_SECONDS"}},
		&cli.IntFlag{Destination: &conf.InsightApplicationLimit, Name: "insight-application-limit", Value: 50, Usage: "Maximum DPI application metric series to export. Set 0 to disable application metrics.", EnvVars: []string{"OMADA_INSIGHT_APPLICATION_LIMIT"}},
